...
typedef struct BlockLinkList
{
    struct BlockNode* head;
    struct BlockNode* tail;
}BlockLinkList,*BlockLinkListPtr;
//队列的操作：
...
void Init_BlockList(BlockLinkListPtr B)
{
    // printf("Init 1\n");
    B->head->next=NULL;//head与第一个结点的链接？？？
    // printf("Init 2\n");
    B->head->height=0;
    B->tail=B->head;
    // printf("Init 3\n");
...
void Insert(BlockLinkListPtr B,BlockNodePtr node)//将newBLK插入本节点的区块链表末尾
{
    node->next=NULL;
    B->tail->next=node;
    B->tail=node;//node成为新的tail
}

BlockNode* get_tail(BlockLinkListPtr B)
{
    return B->tail;
}

int check_hash_conflict(BlockLinkListPtr B,char *hash)//判断该区块是否与已有区块冲突
{
    printf("in check hash\n");
    BlockNodePtr p=B->head->next;
        // printf("in check hash1\n");

    while(p)
    {
            // printf("in check hash loop....\n");

        if(strcmp(p->hash,hash)==0)
        {
            return 1;
        }
        p=p->next;
    }
        // printf("in check hash3\n");
...
int get_block_list_length(BlockLinkListPtr block_list){
...
//根据交易id 查询一个交易
void Query_txid (BlockLinkListPtr block_list,char *txid)
{
    printf("in [query_txid]\n");

    BlockNodePtr p=block_list->head->next;//怎么指向第一个结点
    // printf("in [query_txid] 2: %s\n",p->transactions);

    printf("in query_txid get_block_list_length : %d", get_block_list_length(block_list));
    

    // BlockNodePtr p= block_list->head->next;
    // int i = 0;
    while(p!=NULL){
        if(strstr(p->transactions,txid)!=NULL)
            {
                printf("[find result!!!!!!!]\nblockheight=%dblockhash=%s\n",p->height,p->hash);
                printf("%s\n",p->transactions);
                return;
            }
        p = p->next;
...
void PrintBlock(BlockLinkListPtr B)
{
    BlockNodePtr p=B->head->next;
    while(p)
    {
        printf("hash=%sprevhash=%sheight=%dtrans=%s\n",p->hash,p->prevhash,p->height,p->transactions);
        p=p->next;
    }
} 
...
int main()
{
    ...
    char* block_msg_queue="D:\\codes\\vscode_c\\shujujiegou\\MiniBlockListSystem\\data\\b\\msg_blk\\";
    char* client_msg_queue="D:\\codes\\vscode_c\\shujujiegou\\MiniBlockListSystem\\data\\b\\msg_clt\\";
    char* block_msg_queue_end = "D:\\codes\\vscode_c\\shujujiegou\\MiniBlockListSystem\\data\\c\\msg_blk\\";
    TranListptr transaction_pool;
    transaction_pool = Init();
    
    // printf("[0.5]transcation poopl length: %d\n", transaction_pool->length); 

    BlockLinkListPtr block_linklist;
    block_linklist=(BlockLinkListPtr)malloc(sizeof(BlockLinkList));
    ...
    while(1)
    {
        ...
        if(rand_val<prob_award)//如果中奖
        {
            ...
            if(pool_datas)
            {
                ...
                BlockNodePtr last_blk=get_tail(block_linklist);
                int height=0;
                char prevhash[100];
                char hash[100];
                ...
                enqueue(block_msg_queue_end,new_blk);//将newBLK以字符串形式“发送”给另一个区块链节点的“区块消息队列”。
            }
            free(pool_datas);
        }
        else//没中奖
        {
            printf("no ward \n");
            int isempty_block = is_empty(block_msg_queue);
            ...
            if(!isempty_block)//如果区块消息队列不空
            {
                ...
                if(msg)
                {
                    ...
                    if(!check_hash_conflict(block_linklist,Get(msg,"prevhash:")))
                    {
                        printf("no conflict\n");
                        BlockNodePtr new_blk;
                        new_blk=(BlockNodePtr)malloc(sizeof(BlockNode));
                        ...
                        Insert(block_linklist, new_blk);//将该区块插入到本节点区块链表末尾
                        printf("last get_block_list_length : %d", get_block_list_length(block_linklist));
                        //打印出现在的区块链看看
                        PrintBlock(block_linklist);

                        // printf("[main]newblock trans=%s\n",new_blk->transactions);
            ...
            else if (!isempty_client)//客户消息队列不空
            {
                ...
                { 
                    ...
                    if(jugge_equal==0)//如果是个交易
                    {
                        ...
                        // TranListptr ttt=transaction_pool->next;
                        //     while(ttt!=NULL)
                        //     {
                        //         printf("[tt]pool have--->>>%s\n",ttt->data);
                        //         ttt=ttt->next; 
                        //     }
                        // printf("msg txid:%s\n",Get(msg,"txid:"));
